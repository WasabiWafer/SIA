    ...
    namespace concurrency
    {
        namespace spsc
        {
            ...
            public:
                ...
                constexpr bool try_pop_back(T& out) noexcept(noexcept(out = T()) && noexcept(value_t().~value_t()))
                {
                    ...
                    if (q.is_own())
                    {
                        if (!this->is_empty())
                        {
                            ...
                            loop<true, Tag>(&value_t::has_value, target_ptr);
                ...
                template <tags::wait Tag = tags::wait::busy, typename... Tys>
                constexpr bool try_emplace_back(Tys&&... args) noexcept(noexcept(T(std::forward<Tys>(args)...)))
                {
                    ...
                    if (q.is_own())
                    {
                        if (!this->is_full())
                        {
                            ...
                            loop<false, Tag>(&value_t::has_value, target_ptr);
                ...
                { return this->try_emplace_back<Tag>(arg); }
                
                template <tags::wait Tag = tags::wait::busy>
                constexpr bool try_push_back(T&& arg) noexcept(noexcept(this->try_emplace_back(std::move(arg))))
                { return this->try_emplace_back<Tag>(std::move(arg)); }
                
                template <tags::wait LoopTag = tags::wait::busy, tags::wait TryTag = tags::wait::busy>
                constexpr void pop_back(T& out) noexcept(noexcept(this->try_pop_back(out)))
                { loop<true, LoopTag>(&ring::try_pop_back<TryTag>, this, out); }
                
                template <tags::wait LoopTag = tags::wait::busy, tags::wait TryTag = tags::wait::busy>
                constexpr void push_back(const T& arg) noexcept(noexcept(this->try_push_back(arg)))
                { loop<true, LoopTag>(static_cast<bool(ring::*)(const T&)>(&ring::try_push_back<TryTag>), this, arg); }
                
                template <tags::wait LoopTag = tags::wait::busy, tags::wait TryTag = tags::wait::busy>
                constexpr void push_back(T&& arg) noexcept(noexcept(this->try_push_back(std::move(arg))))
                { loop<true, LoopTag>(static_cast<bool(ring::*)(T&&)>(&ring::try_push_back<TryTag>), this, std::move(arg)); }

                template <tags::wait LoopTag = tags::wait::busy, tags::wait TryTag = tags::wait::busy, typename... Tys>
                constexpr void emplace_back(Tys&&... args) noexcept(noexcept(this->try_emplace_back(std::forward<Tys>(args)...)))
                { loop<true, LoopTag>(&ring::try_emplace_back<TryTag, Tys...>, this, std::forward<Tys>(args)...); }
                ...
                constexpr bool try_emplace_front(Tys&&... args) noexcept(noexcept(T(std::forward<Tys>(args)...)))
                {
                    ...
                    if (q.is_own())
                    {
                        if (!this->is_full())
                        {
                            value_t* target_ptr = this->raw_address(comp.m_begin->fetch_sub(1, mem_order) - 1);
                            loop<false, Tag>(&value_t::has_value, target_ptr);
                            target_ptr->emplace(std::forward<Tys>(args)...);
                            return true;
                        }
